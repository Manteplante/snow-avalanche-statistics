"""Flom — interactive map of NVE's flood-avalanche-event registry, reads the
table 03_notebooks/05_clean_nve_flood_avalanche_events.ipynb saved as
'nve_flood_avalanche_events_clean'.

Mass-carrying flood events (skredType 171 — a small category, ~34 records)
...
df = storage.load("nve_flood_avalanche_events_clean")

if df.empty:
    st.info(f"No `nve_flood_avalanche_events_clean` table yet. Reading from `{storage.describe()}`.")
    st.stop()
