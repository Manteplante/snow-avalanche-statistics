"""Steinskred — interactive map of NVE's rock-avalanche-event registry, reads
the table 03_notebooks/06_clean_nve_rock_avalanche_events.ipynb saved as
'nve_rock_avalanche_events_clean'.

Rockfall and rockslide events (skredType 110-113 — the largest category,
~54,000 records) from NVE's national landslide/avalanche event database — a
hazard registry, not curated accident reports. NVE data is never part of
`records` (see backend/transform.py and 05_tests/test_records_excludes_nve.py)
— this page's table comes from the notebook cleaning the raw NVE file
directly.
...
df = storage.load("nve_rock_avalanche_events_clean")

if df.empty:
    st.info(f"No `nve_rock_avalanche_events_clean` table yet. Reading from `{storage.describe()}`.")
    st.stop()
...
# object per row, which doesn't scale to tens of thousands of points (this is
# the biggest NVE category, ~54,000 records).
