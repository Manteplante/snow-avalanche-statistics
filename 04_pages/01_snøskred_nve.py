"""Snøskred NVE — interactive map of NVE's avalanche-event registry, reads
the table 03_notebooks/03_clean_nve_avalanche_events.ipynb saved as
'nve_avalanche_events_clean'.

This is NVE's national hazard registry (~35,000 events, mostly with no
casualties), not varsom.no's curated accident data on the other Snøskred
page — kept as a separate page reading a separate table. NVE data is never
part of `records` (see backend/transform.py and
05_tests/test_records_excludes_nve.py) — this page's table comes from the
notebook cleaning the raw NVE file directly.

Uses Kartverket's topographic WMTS tiles (verified live), same as the other
two map pages. Uses FastMarkerCluster rather than the Marker+Icon+MarkerCluster
combination the other two pages use: at ~34,600 points, building one Python
object per marker embeds a document too large for the browser to render —
FastMarkerCluster ships the raw data instead and builds markers in JS.
"""
...
df = storage.load("nve_avalanche_events_clean")

if df.empty:
    st.info(f"No `nve_avalanche_events_clean` table yet. Reading from `{storage.describe()}`.")
    st.stop()
...
# `callback` builds each marker there — instead of ~34,600 individual Python
# Marker/Popup objects, which produced a document too large to render.
...
# to L.AwesomeMarkers.icon() on the other two pages — verified against the
# installed folium package's source rather than assumed.
CALLBACK = """
function (row) {
    function esc(v) {
